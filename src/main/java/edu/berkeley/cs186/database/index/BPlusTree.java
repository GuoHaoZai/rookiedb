...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        root.put(key, rid).ifPresent(rootRightSibling -> {
            InnerNode newRoot = new InnerNode(metadata,
                                              bufferManager,
                                              Arrays.asList(rootRightSibling.getFirst()),
                                              Arrays.asList(root.getPage().getPageNum(),
                                                            rootRightSibling.getSecond()),
                                              lockContext);
            updateRoot(newRoot);
        });
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (left != this.root || left.scanAll().hasNext()) { // 叶子下的迭代器能后进行hasNext 证明有数据
            throw new BPlusTreeException("cannot bulk load into nonempty tree");
        }
        while (data.hasNext()) {
            this.root.bulkLoad(data, fillFactor).ifPresent(rootRightSibling -> {
                InnerNode newRoot = new InnerNode(this.metadata,
                                                  this.bufferManager,
                                                  Arrays.asList(rootRightSibling.getFirst()),
                                                  Arrays.asList(root.getPage().getPageNum(),
                                                                rootRightSibling.getSecond()),
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            return this.iter.hasNext() || this.leaf.getRightSibling().isPresent();
        }
        ...
        public RecordId next() {
            ...
            Optional<LeafNode> rightSibling = this.leaf.getRightSibling();
            if (rightSibling.isPresent()) {
                this.leaf = rightSibling.get();
                this.iter = rightSibling.get().scanAll();
                return next();
            }
